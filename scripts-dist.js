...
$(document).ready(function () {
	...
	if (isFirefox) {
		prefix = '-moz-';
	} else if (isIE) {} else if (isChrome || isSafari) {
		prefix = '-webkit-';
	}
...
$(window).load(function () {
	...
	data.on('value', function (v) {
		var work = v.val().work;
		$(".work-position").html(work.Position);
		$(".work-place").html(work.Company);

		var portfolio = v.val().portfolio;
		var html = '';

		var _loop = function _loop(i) {
			var l = portfolio[i].technologies.length - 1;
			portfolio[i].technologies.forEach(function (v, index) {
				if (index == 0) {
		...
		var alltime = $(".firebase-item").length * 600;
